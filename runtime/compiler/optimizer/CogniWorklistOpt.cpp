	    ...
	    if(found){
	      ...
	      //one item is enough
			...
			try{
			  ...
			  client->writeClient(TCP::ClientMsgType::clientRequestInitAnalysis, compileeClass, feMethod->classNameLength());
			  ...
			  for (int32_t i = 0; i < comp()->getNumInlinedCallSites(); ++i) {
				...
				client->writeClient(TCP::ClientMsgType::clientRequestInitAnalysis, comp()->getInlinedResolvedMethod(i)->classNameChars(), comp()->getInlinedResolvedMethod(i)->classNameLength());
				client->writeClient(TCP::ClientMsgType::clientRequestInitAnalysis, "\n", 0);
